...
function OnAnyChatConnect(bSuccess, errorcode) {
	...
	//alert("loginFailedFlag:"+loginFailedFlag+"-----errorcode:"+errorcode)
	  AddLog("连接回调");
    ...
    $("#myModalLabel5").html("链接成功<br>正在生成表单数据<br>请稍后。。。")	
    AnyChatUpFile();
	} else {
		...
		if(loginFailedFlag == "1"){
		  ...
		  $("#myModalLabel5").html("表单上送失败<br>服务器链接失败")			 
	...
	if(obj.mode == "form")
	{
		if(videoInitFlag == 1)
	    {
		 ...
		 iTaskId = BRAC_TransFile(0, obj.perImagePath, 0, 0, 128);//上传日志	
				...
				$("#myModalLabel5").html("表单上送失败</br>是否从新发送？<br>"+obj.perImagePath)			 
			$("#commit5").css("display","none");
			$("#commit6").css("display","none");
			$("#commit7").css("display","block");
			$('#myModal5').modal('show');
		...
		if(videoInitFlag == 1)
		{
		 ...
		 iTaskId = BRAC_TransFile(0, obj.perIDCardImagePath, 0, 0, 128);//上传日志	
			...
			$("#myModalLabel5").html("表单上送失败</br>是否从新发送2？")			 
			$("#commit5").css("display","none");
			$("#commit6").css("display","none");
			$("#commit7").css("display","block");
			$('#myModal5').modal('show');
